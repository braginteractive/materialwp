...
function materialwp_post_nav() {
	...
	<nav class="navigation post-navigation" role="navigation">
		...
		<div class="nav-links">
			<?php
				previous_post_link( '<div class="nav-previous">%link</div>', _x( '<span class="meta-nav">&larr;</span>&nbsp;%title', 'Previous post link', 'materialwp' ) );
				next_post_link(     '<div class="nav-next">%link</div>',     _x( '%title&nbsp;<span class="meta-nav">&rarr;</span>', 'Next post link',     'materialwp' ) );
			?>
		</div><!-- .nav-links -->
	</nav><!-- .navigation -->
	<?php
...
function materialwp_posted_on() {
	...
	$posted_on = sprintf(
		_x( 'Posted on %s', 'post date', 'materialwp' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		_x( 'by %s', 'post author', 'materialwp' ),
...
function materialwp_entry_footer() {
	...
	if ( 'post' == get_post_type() ) {
		...
		if ( $categories_list && materialwp_categorized_blog() ) {
			printf( '<span class="cat-links">' . __( 'Posted in %1$s', 'materialwp' ) . '</span>', $categories_list );
		}
